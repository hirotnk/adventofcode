...
start() ->
  L = lists:reverse(get_data_list()),
  W = lists:sum(L) div 4,
  C = combi(W,L,[]),
  get_min_qe(C, length(hd(C)), []).

get_data_list() ->
  {ok, Bin} = file:read_file("aoc24.txt"),
  [ binary_to_integer(B)
     || B <- binary:split(Bin, [<<"\n">>], [global]), size(B) > 0].

get_min_qe([H|T], Len, Acc) when length(H) == Len ->
  get_min_qe(T, Len, [{lists:foldl(fun(E,P) -> E*P end, 1, H),H}|Acc]);
get_min_qe([H|T], Len, Acc) -> 
  get_min_qe(T, Len, Acc);
get_min_qe([], Len, Acc) -> lists:min(Acc).

combi(0, _, Acc) -> [Acc];
combi(Target, _, _Acc) when Target < 0 -> [];
combi(_, [], _Acc) -> [];
combi(Target, L, Acc) -> 
  lists:flatmap( fun(E) -> combi(Target - E, [R || R<-L, R>E], [E|Acc]) end, L).
