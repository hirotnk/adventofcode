...
find_house(Max) ->
  ets:new(house, [set, public, named_table]),
  deliver(1, 1, 1, (Max+1) div 11),
  find_house_in(1,(Max+1) div 11).

find_house_in(House, Max) ->
  Val = ets:lookup_element(house, House, 2), 
  case Val >= Max of
    true ->
      ets:delete(house),
      io:format("The first house to get ~p presents is ~p~n", [Max, House]);
    false -> find_house_in(House+1, Max)
  end.
  
deliver(Elf, _House, Visits, Max) when Elf >= Max -> ok;

deliver(1, House, Visits, Max) when Visits >= Max ->
  io:format("elf1 is done~n"),
  deliver(2, 2, 1, Max);
deliver(1, House, Visits, Max) when Visits > 50 ->
  ets:insert(house, {House, 0}),
  deliver(1, House+1, Visits+1, Max);
deliver(Elf, House, Visits, Max) when Visits > 50 ->
  deliver(Elf+1, Elf+1, 1, Max);
deliver(Elf, House, Vists, Max) when House >= Max ->
  deliver(Elf+1, Elf+1, 1, Max);

deliver(1, House, Visits, Max) ->
  ets:insert(house, {House, 11}),
  deliver(1, House+1, Visits+1, Max);

deliver(Elf, House, Visits, Max) ->
  Val = ets:lookup_element(house, House, 2),
  ets:insert(house, {House, Val+Elf}),
  deliver(Elf, House+Elf, Visits+1, Max).
...
%   House 4 got 70 presents.  House 5 got 60 presents.  House 6 got 120 presents.
%   House 7 got 80 presents.  House 8 got 150 presents.  House 9 got 130 presents.
