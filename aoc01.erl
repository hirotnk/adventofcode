...
start() ->
  solve(get_data_list(), 0, 1).

get_data_list() ->
  {ok, Bin} = file:read_file("aoc01.txt"),
  [Data] = [binary_to_list(B) || B <- binary:split(Bin, [<<"\n">>], [global]), size(B) > 0],
  Data.

solve([], _Acc, Pos) -> Pos;
solve([$(|T], Acc, Pos) -> solve(T, Acc+1, Pos + 1);
solve([$)|T], Acc, Pos) ->
  if
    (Acc -1) == -1 -> Pos;
    true -> solve(T, Acc-1, Pos + 1)
  end.
